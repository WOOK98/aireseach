...
async function publishFromFile(filePath) {
  ...
  const { title, tags = [], status, thumbnail, description } = parsed.attributes;
...
function mdToHtml(md, { title, thumbnail, description } = {}) {
  ...
  // 标题
  html = html.replace(/^### (.+)$/gm, '<h3 style="font-size:20px;font-weight:700;margin:32px 0 16px;color:#1a1a1a;">$1</h3>');
  html = html.replace(/^## (.+)$/gm, '<h2 style="font-size:24px;font-weight:700;margin:40px 0 16px;color:#1a1a1a;border-bottom:2px solid #e5e5e5;padding-bottom:8px;">$1</h2>');
  html = html.replace(/^# (.+)$/gm, '<h1 style="font-size:32px;font-weight:800;margin:48px 0 24px;color:#0a0a0a;">$1</h1>');
  ...
  // 引用块
  html = html.replace(/^> (.+)$/gm, '<blockquote style="border-left:4px solid #2563eb;padding:12px 20px;margin:20px 0;background:#f8fafc;font-style:italic;color:#475569;">$1</blockquote>');
  ...
  html = html.replace(/\|(.+)\|\n\|[-| :]+\|\n((?:\|.+\|\n?)+)/g, (match, header, rows) => {
    ...
    let table = '<table style="width:100%;border-collapse:collapse;margin:20px 0;font-size:15px;">';
    table += '<thead><tr>';
  ...
  // 无序列表
  html = html.replace(/^- (.+)$/gm, '<li style="margin:8px 0;line-height:1.7;">$1</li>');
  html = html.replace(/(<li[\s\S]*?<\/li>)/g, '<ul style="padding-left:24px;margin:16px 0;">$1</ul>');
  
  // 有序列表
  html = html.replace(/^\d+\. (.+)$/gm, '<li style="margin:8px 0;line-height:1.7;">$1</li>');
  
  // 复选框列表
  html = html.replace(/- \[x\] (.+)$/gm, '<div style="margin:8px 0;padding:8px 12px;background:#f0fdf4;border-radius:6px;">✅ $1</div>');
  html = html.replace(/- \[ \] (.+)$/gm, '<div style="margin:8px 0;padding:8px 12px;background:#f8fafc;border-radius:6px;border:1px solid #e2e8f0;">⬜ $1</div>');
  
  // 水平线
  html = html.replace(/^---$/gm, '<hr style="border:none;border-top:2px solid #e5e5e5;margin:40px 0;">');
  ...
  // 添加封面图
  const heroImg = thumbnail ? `<div style="margin:0 0 32px;"><img src="${thumbnail}" alt="${title}" style="width:100%;max-height:400px;object-fit:cover;border-radius:12px;" /></div>` : '';
  
  // 添加描述
  const descHtml = description ? `<p style="font-size:18px;color:#64748b;line-height:1.6;margin:0 0 32px;font-style:italic;">${description}</p>` : '';
...
if (args.includes('--update')) {
  ...
  const result = await updatePost(postId, { title, content: html, labels: tags });
