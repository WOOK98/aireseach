...
// Import build-time generated constants (no readFileSync, no disk access)
import { DEEP_DIVE_METHODOLOGY, FILING_METHODOLOGY } from "./generated";

export { DEEP_DIVE_METHODOLOGY, FILING_METHODOLOGY };

// ── Shared constraints (inlined constants — no disk dependency) ──────────────

/**
 * Hard rules that apply to ALL analysis modes.
 * These are the invariant constraints — the skill defines them, this
 * module re-exports them as a composable prompt fragment.
 */
...
/**
 * Monitor JSON schema required by the shared contract.
 * Used by both the finance/generate and committee/generate endpoints.
 */
...
// ── Mode-specific perspective slices ─────────────────────────────────────────

/**
 * Each website mode is a "perspective slice" — it selects which parts
 * of the methodology to emphasize, not a separate analysis contract.
 */
