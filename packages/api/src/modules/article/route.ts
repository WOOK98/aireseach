...
// TEMP DEBUG: full article pipeline with detailed error info (remove after #205)
articleRoute.get("/debug-article/:query", async (c) => {
  ...
  try {
    resolution = await cachedResolveEntity(query);
    steps.entityResolution = { ok: resolution.ok, mode: resolution.mode };
  } catch (err) {
    const ticker = query.trim().toUpperCase();
    resolution = {
      ok: true,
      ...
      price: null,
      currency: null,
      entityLock: `ENTITY: ${ticker}`,
    };
    steps.entityResolution = {
      ok: true,
      mode: "ticker",
      fallback: true,
      error: String(err),
  ...
  const [yahooResult, imaKnowledge] = await Promise.allSettled([
    resolution.ok && resolution.mode === "ticker"
      ...
      : Promise.resolve(null),
    searchImaKnowledge(symbol, { limit: 6 }),
  ]);
  steps.yahooFinance = {
    status: yahooResult.status,
    hasValue: !!(yahooResult.status === "fulfilled" && yahooResult.value),
  };
  steps.imaKnowledge = {
    status: imaKnowledge.status,
    count: imaKnowledge.status === "fulfilled" ? imaKnowledge.value.length : 0,
  };

  if (yahooResult.status === "fulfilled" && yahooResult.value) {
    financials = yahooResult.value;
  }
  const imaHits = imaKnowledge.status === "fulfilled" ? imaKnowledge.value : [];
  imaContext = formatImaKnowledgeForPrompt(imaHits);

  // Step 3: Data gate
  const spine = buildInputSpine(financials, "", imaContext, resolution);
  steps.dataGate = { ...spine, passes: hasVerifiedInput(spine) };

  if (!hasVerifiedInput(spine)) {
    return c.json({ steps, result: "degraded at data gate" });
  }
  ...
  try {
    ...
    const complianceHit = PROHIBITED_ARTICLE_PATTERN.test(text);
    steps.compliance = { passes: !complianceHit };

    // Step 6: JSON parse
    try {
      const parsed = parseArticleJson(text);
      steps.jsonParse = { ok: true, type: typeof parsed };

      // Step 7: Schema validation
      const validation = researchArticleSchema.safeParse(parsed);
      steps.schemaValidation = {
      ...
      steps.jsonParse = { ok: false, error: String(parseErr) };
    }

    return c.json({ steps, result: "completed" });
  } catch (llmErr) {
    steps.llmError =
      llmErr instanceof Error
        ? { message: llmErr.message, name: llmErr.name }
        : String(llmErr);
    return c.json({ steps, result: "llm error" });
  }
});
